...
function getResults() {
  getResponse("attackInfo.json", function(responseText) {
    ...
    var aps = "";
    var clients = "";
    var tr = "<tr><th>搞事类型</th><th>状态</th><th>启/停</th></tr>";
	...
	if(res.randomMode == 1) randomBtn.innerHTML = "随机关";
	else randomBtn.innerHTML = "随机开";
    ...
    for (var i = 0; i < res.attacks.length; i++) {
      ...
      if (res.attacks[i].running) tr += "<td><button class='select' onclick='startStop(" + i + ")'>停</button></td>";
      else tr += "<td><button class='select' onclick='startStop(" + i + ")'>启</button></td>";
	...
	if(typeof res.ssid != 'undefined'){
		...
		var tr = "<tr><th>名称</th><th></th><th>删除</th></tr>";
		for (var i = 0; i < data.length; i++) {
		  tr += "<tr>";
		  tr += "<td>" + escapeHTML(data[i][0]) + "</td>";
		  if(data[i][1] == 1) tr += "<td>&#128274;</td>";
		  else tr += "<td></td>";
		  tr += '<td><button class="button-warn" onclick="deleteSSID(' + i + ')">删</button></td>';
		  tr += "</tr>";
		}
    ...
    showMessage("attackInfo.json加载错误");
  });
}

function startStop(num) {
  getResponse("attackStart.json?num=" + num, function(responseText) {
	getE("status"+num).innerHTML = "加载中...";
    if (responseText == "true") getResults();
    else showMessage("attackStart.json响应错误");
  });
}
...
function addSSID() {
	...
	if(_ssidName.length > 0){
		if(data.length >= 64) showMessage("SSID；列表满了:(", 2500);
		else{
			saved.innerHTML = "";
